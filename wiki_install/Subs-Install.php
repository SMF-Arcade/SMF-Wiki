...
class DatabaseInstaller
{
	 ...
	 *
	 */
	function  __construct($tables, $columnRename, $addSettings, $permissions, $prefix = '{db_prefix}')
	{
		$this->tables = $tables;
	...
	function doTables()
	{
		...
		foreach ($this->tables as $table)
		{
			...
			$tableExists = in_array(str_replace('{db_prefix}', $db_prefix, $this->prefix) . $table_name, $existingTables);
	...
	function doSettings()
	{
		...
		foreach ($this->addSettings as $variable => $value)
		{
	...
	function doPermission()
	{
		...
		foreach ($this->permissions as $permission => $default)
		{
